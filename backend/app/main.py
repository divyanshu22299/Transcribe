...
from app.models import (
    TranscriptionResult, Segment, AutoFixRequest, ExportRequest, BatchTask, AudioAnalysis, WordConfidence, QCError
)
...
# Initialize Neon PostgreSQL tables on launch
...
async def upload_audio(file: UploadFile = File(...)):
    ...
    audio_id = Path(file.filename).stem
...
async def list_projects():
    ...
    if not session:
        return {"projects": []}
    try:
        projs = session.query(DBProject).order_by(DBProject.updated_at.desc()).all()
        result = []
        for p in projs:
            result.append({
                "id": p.id,
...
@app.post("/api/lint")
async def lint_segments_endpoint(
    payload: dict
):
    """Re-lint segment list after manual edits."""
    ...
    linted_segments, score, total_errors, total_warnings = lint_dataset(
        segments, language=language, script=script
    )
...
async def autofix_endpoint(
    ...
    """Apply 1-click auto-fixes on segments."""
    segments_raw = payload.get("segments", [])
